...
# 设置参数
maximum_forklifts = 22

# 计算每班的工人数量
shift_forklifts_am = df['forklift_dock_am'].values + df['forklift_inbound_am'].values + df['forklift_outbound_am'].values
shift_forklifts_pm =  df['forklift_dock_pm'].values + df['forklift_inbound_pm'].values + df['forklift_outbound_pm'].values
...
# 保存图像
plt.savefig('Unused forklifts_3obj.pdf', dpi=800, pad_inches=0)
plt.show()
